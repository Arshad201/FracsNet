...
const Head = async({data}) => {
    ...
    const settingItemsForPostAdmin = [
        ...
        {
            type: "link",
            link: `/create?update=job&resource_id=${data._id}`,
            text: "Edit",
        },
    ...
    let settingsItemsForEveryone = [
        ...
        },
        {
            ...
            action: async (data) => {
                "use server";
                const postLink = `${process.env.BASE_URL}/job/${data._id}`
                return {postLink}
            }
        },
    ...
    let settingsItems = [];
    if(data.employer._id == loggedInUser._id) {
